...
const statusConfig = {
  ...
  in_progress: { icon: AlertCircle, color: "text-yellow-600", bg: "bg-yellow-100" },
        ...
        <CardContent className="space-y-4">
          {data.recentAttempts.length === 0 ? (
            ...
            data.recentAttempts.map((attempt) => {
              const StatusIcon = statusConfig[attempt.status as keyof typeof statusConfig]?.icon || AlertCircle;
              const statusStyle = statusConfig[attempt.status as keyof typeof statusConfig] || statusConfig.in_progress;
        ...
        <Card key={cardIndex}>
          ...
          <CardContent className="space-y-4">
            {Array.from({ length: 5 }).map((_, itemIndex) => (
              <div key={itemIndex} className="flex items-center justify-between p-3 rounded-lg border">
                <div className="flex items-center gap-3">
