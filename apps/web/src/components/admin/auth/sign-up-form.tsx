...
export const SignUpForm = () => {
	...
	const { mutateAsync: signUpWithEmail, isPending } = useMutation({
		mutationKey: ["sign-up", "email"],
		mutationFn: async (
			formData: Omit<AdminSignupSchemaType, "confirmPassword">,
		) => {
		...
		<div className="mx-auto grid w-full max-w-sm gap-6">
			...
			<Form {...form}>
				<form className="grid gap-4" onSubmit={form.handleSubmit(onSubmit)}>
					...
					<Button className="group w-full" disabled={isPending} size="lg">
						{isPending ? (
							<>
